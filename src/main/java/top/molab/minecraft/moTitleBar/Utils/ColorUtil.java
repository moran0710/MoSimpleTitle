...
/**
 * 颜色解析工具类
 * 基于 CarmJos/EasyPlugin 的 ColorParser 实现修改，遵循 MIT 协议
 * 源码地址：<a href="https://github.com/CarmJos/EasyPlugin/blob/master/.../ColorParser.java">...</a>
 *
 * 颜色解析器。
...
public class ColorUtil {
    ...
    public static @NotNull List<String> parse(@NotNull Collection<String> texts) {
        return texts.stream().map(ColorUtil::parse).collect(Collectors.toList());
    }
    ...
    public static @NotNull String parseGradientColor(@NotNull String text) {
        ...
        Matcher matcher = ColorUtil.GRADIENT_PATTERN.matcher(text);
        while (matcher.find()) colors.add(matcher.group(1));

        if (colors.isEmpty()) return text; // 无渐变颜色，直接跳出

        String[] parts = ColorUtil.GRADIENT_PATTERN.split(text);
    ...
    public static @NotNull String gradientText(@NotNull String text,
        ...
        Matcher matcher = ColorUtil.FORMAT_PATTERN.matcher(text);
        while (matcher.find()) {
